...
;;; config.el --- Mi configuración personal de Doom Emacs -*- lexical-binding: t; -*-
;;; Commentary:
;;
;;      ██╗██████╗  █████╗  ██████╗██╗  ██╗███████╗ ██████╗ ██████╗ ██╗  ██╗██╗   ██╗███████╗
;;      ██║██╔══██╗██╔══██╗██╔════╝██║  ██║██╔════╝██╔════╝██╔═══██╗╚██╗██╔╝╚██╗ ██╔╝╚══███╔╝
...
* User configuration
This Section is to configure my user email, and the main =doom= configuration like, cursor, window frame, line numbers configuration
#+begin_src emacs-lisp
...
*** Capture templates for contatcs
#+begin_src emacs-lisp
...
This are my tags alist to use in my personal agenda file:
...
The code below, is for bind keys in org-agenda-mode, I realize that I need to unbind the key first, then bind the key to its new command.
...
This code modifies the main entry of the org agenda, and sets some variables.
...
*** Custom agenda scripts
This custom =elisp= code is 
#+begin_src emacs-lisp
...
* Custom packages.
These are my main emacs packages that I use the most. These are definitely the most useful packages for my work flow.
** Built-in
#+begin_src emacs-lisp
;; Server Mode
(use-package! server
...
** Denote
The best package ever created, period.
#+begin_src emacs-lisp
;;; DENOTE
(use-package! denote
