...
;; DEPRECATED 2023-12-26: The `jp-simple-goto-definition' is a good
...
;;
;; ;;;###autoload
;; (defun jp-simple-goto-definition (symbol)
...
;;
;; (defun jp-simple--number-operate (number amount operation)
...
;;
;; (defun jp-simple--number-replace (number amount operation)
;;   "Perform OPERATION on NUMBER at point by AMOUNT."
;;   (when-let* ((bounds (bounds-of-thing-at-point 'number))
;;               (replacement (jp-simple--number-operate number amount operation)))
;;     (delete-region (car bounds) (cdr bounds))
;;     (save-excursion
;;       (insert (number-to-string replacement)))))
;;
;; ;;;###autoload
;; (defun jp-simple-number-increment (number amount)
...
;;   (jp-simple--number-replace number amount :increment))
;;
;; ;;;###autoload
;; (defun jp-simple-number-decrement (number amount)
...
;;     (number-at-point)
;;     (prefix-numeric-value current-prefix-arg)))
;;   (jp-simple--number-replace number amount :decrement))
...
;; (defcustom jp-simple-line-prefix-strings '(">" "+" "-")
;;   "List of strings used as line prefixes.
;; The command which serves as the point of entry is
;; `jp-simple-insert-line-prefix'."
;;   :type '(repeat string)
;;   :group 'jp-simple)
;;
;; (defun jp-simple--line-prefix-regexp (&optional string)
;;   "Format regular expression for `jp-simple--line-prefix-p'.
;; With optional STRING use it directly.  Else format the regexp by
;; concatenating `jp-simple-line-prefix-strings'."
;;   (if string
;;       (format "^%s " string)
;;     (format "^[%s] " (apply #'concat jp-simple-line-prefix-strings))))
;;
;; (defun jp-simple--line-prefix-p (&optional string)
...
;;   (save-excursion
;;     (goto-char (line-beginning-position))
;;     (looking-at (jp-simple--line-prefix-regexp string))))
;;
;; (defun jp-simple--line-prefix-insert (string)
...
;;     (insert " ")))
;;
;; (defun jp-simple--line-prefix-infer-string ()
;;   "Return line prefix string if it matches `jp-simple--line-prefix-p'."
;;   (when (jp-simple--line-prefix-p)
;;     (string-trim
;;      (buffer-substring-no-properties (match-beginning 0) (match-end 0)))))
;;
;; (defun jp-simple--line-prefix-toggle (string)
;;   "Insert or remove STRING at the beginning of the line."
;;   (if (jp-simple--line-prefix-p string)
;;       (delete-region (match-beginning 0) (match-end 0))
;;     (jp-simple--line-prefix-insert string)))
;;
;; (defvar jp-simple--line-prefix-history nil
;;   "Minibuffer history of `jp-simple--line-prefix-prompt'.")
;;
;; (defun jp-simple--line-prefix-prompt ()
;;   "Prompt for string to use as line prefix.
;; Provide `jp-simple-line-prefix-strings' as completion
;; candidates, though accept arbitrary input."
;;   (let ((default (car jp-simple--line-prefix-history)))
;;     (completing-read
;;      (format-prompt "Select line prefix" default)
;;      jp-simple-line-prefix-strings
;;      nil nil nil
;;      'jp-simple--line-prefix-history default)))
;;
;; (defun jp-simple-line-prefix-infer-or-prompt ()
;;   "Infer string for line prefix or prompt for one."
;;   (or (jp-simple--line-prefix-infer-string)
;;       (jp-simple--line-prefix-prompt)))
;;
;; ;;;###autoload
;; (defun jp-simple-insert-line-prefix-dwim (string)
;;   "Toggle presence of STRING at the beginning of the line.
;;
;; When called interactively try to infer STRING based on the line
;; prefix.  If one is found among `jp-simple-line-prefix-strings',
;; perform a removal outright.
;;
;; If no string can be inferred, prompt for STRING among
;; `jp-simple-line-prefix-strings'.  Accept arbitrary strings at
;; the prompt.
;;
;; When the region is active, toggle the presence of STRING for each
;; line in the region."
;;   (interactive (list (jp-simple-line-prefix-infer-or-prompt)))
...
;;           (jp-simple--line-prefix-toggle string)
;;           (forward-line 1)))
;;     (jp-simple--line-prefix-toggle string)))
