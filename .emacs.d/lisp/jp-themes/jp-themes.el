...
This is used by the commands `jp-themes-select' and
`jp-themes-load-random'."
  :type 'hook
  :package-version '(jp-themes . "0.2.0")
  :group 'jp-themes)

(defcustom jp-themes-disable-other-themes t
  "Disable all other themes when loading a Ef theme.

When the value is non-nil, the commands `jp-themes-toggle' and
`jp-themes-select' will disable all other themes while loading
...
set this variable to a nil value."
  :group 'jp-themes
  :package-version '(jp-themes . "0.11.0")
  :type 'boolean)

(defcustom jp-themes-to-toggle nil
  "Specify two `jp-themes' for `jp-themes-toggle' command.
The variable `jp-themes-collection' contains the symbols of all
  ...
  :type `(choice
          ...
          (list :tag "Pick two themes to toggle between"
                (choice :tag "Theme one of two"
                        ,@(mapcar (lambda (theme)
                                    (list 'const theme))
                                  jp-themes-collection))
                (choice :tag "Theme two of two"
                        ,@(mapcar (lambda (theme)
                                    (list 'const theme))
                                  jp-themes-collection))))
  :package-version '(jp-themes . "0.3.0")
  :group 'jp-themes)

(defconst jp-themes-weights
  '( thin ultralight extralight light semilight regular medium
     semibold bold heavy extrabold ultrabold)
  "List of font weights.")

(defconst jp-themes--headings-choice
  ...
  "Refer to the doc string of `jp-themes-headings'.
This is a helper variable intended for internal use.")

(defcustom jp-themes-headings nil
...
- The symbol of a weight attribute adjusts the font of the
  ...
  symbols are defined in the variable `jp-themes-weights'.  The
...
level.  For example:
  ...
  :group 'jp-themes
  :package-version '(jp-themes . "0.10.0")
  :type `(alist
          :options ,(mapcar (lambda (el)
                              (list el jp-themes--headings-choice))
                            '(0 1 2 3 4 5 6 7 8 t agenda-date agenda-structure))
          :key-type symbol
          :value-type ,jp-themes--headings-choice)
  :link '(info-link "(jp-themes) Option for headings"))

(defcustom jp-themes-mixed-fonts nil
...
Protesilaos))."
  :group 'jp-themes
  :package-version '(jp-themes . "0.4.0")
  :type 'boolean
  :link '(info-link "(jp-themes) Enable mixed fonts"))

(defcustom jp-themes-variable-pitch-ui nil
...
ELPA (by Protesilaos))."
  ...
  :package-version '(jp-themes . "0.4.0")
  :type 'boolean
  :link '(info-link "(jp-themes) UI typeface"))
...
THEME-NAME is one of the symbols in `jp-themes-collection'.

Individual theme overrides take precedence over these common
overrides.

To preview the palette entries, use `jp-themes-preview-colors' or
`jp-themes-preview-colors-current' (read the documentation for
further details)."
  :group 'jp-themes
  :package-version '(jp-themes . "1.0.0")
  :type '(repeat (list symbol (choice symbol string)))
  :link '(info-link "(jp-themes) Palette overrides"))

;;; Helpers for user options

(defun jp-themes--warn (option)
...
(defun jp-themes--list-or-warn (option)
  "Return list or nil value of OPTION, else `jp-themes--warn'."
  (let* ((value (symbol-value option)))
    (if (or (null value) (listp value))
        value
      (jp-themes--warn option))))

(defun jp-themes--fixed-pitch ()
  "Conditional application of `fixed-pitch' inheritance."
  (when jp-themes-mixed-fonts
    (list :inherit 'fixed-pitch)))

(defun jp-themes--variable-pitch-ui ()
  "Conditional application of `variable-pitch' in the UI."
  (when jp-themes-variable-pitch-ui
    (list :inherit 'variable-pitch)))

(defun jp-themes--property-lookup (properties alist-key list-pred default)
...
(defun jp-themes--weight (list)
  "Search for `jp-themes--heading' weight in LIST."
  (catch 'found
    (dolist (elt list)
      (when (memq elt jp-themes-weights)
        (throw 'found elt)))))

(defun jp-themes--heading (level)
  "Conditional styles for `jp-themes-headings' per LEVEL heading."
  (let* ((key (alist-get level jp-themes-headings))
         (style (or key (alist-get t jp-themes-headings)))
         (style-listp (listp style))
         (properties style)
         (var (when (and style-listp (memq 'variable-pitch properties)) 'variable-pitch))
         (weight (when style-listp (jp-themes--weight style))))
    (list :inherit
          (cond
          ...
          :height
          (if style-listp
              (jp-themes--property-lookup properties 'height #'floatp 'unspecified)
            'unspecified)
          :weight
...
(defun jp-themes--retrieve-palette-value (color palette)
...
This function is used in the macros `jp-themes-theme',
`jp-themes-with-colors'."
  (let ((value (car (alist-get color palette))))
    (cond
     ...
     ((and (symbolp value)
      ...
      (jp-themes--retrieve-palette-value value palette))
     (t
      'unspecified))))

(defun jp-themes-get-color-value (color &optional overrides theme)
...
With optional THEME as a symbol among `jp-themes-collection', use
...
symbol, which is safe when used as a face attribute's value."
  (if-let* ((palette (if theme
                         (jp-themes--palette-value theme overrides)
                       (jp-themes--current-theme-palette overrides)))
            (value (jp-themes--retrieve-palette-value color palette)))
      value
    'unspecified))

(defun jp-themes--list-enabled-themes ()
  "Return list of `custom-enabled-themes' with jp- prefix."
  (seq-filter
   (lambda (theme)
     (string-prefix-p "jp-" (symbol-name theme)))
...
(defun jp-themes--list-known-themes ()
  "Return list of `custom-known-themes' with jp- prefix."
  (jp-themes--enable-themes)
  (seq-filter
   (lambda (theme)
     (string-prefix-p "jp-" (symbol-name theme)))
   custom-known-themes))

(defun jp-themes--current-theme ()
  "Return first enabled Ef theme."
  (car (or (jp-themes--list-enabled-themes)
           (jp-themes--list-known-themes))))

(defun jp-themes--palette-symbol (theme &optional overrides)
...
(defun jp-themes--palette-value (theme &optional overrides)
  "Return palette value of THEME with optional OVERRIDES."
  (let ((base-value (symbol-value (jp-themes--palette-symbol theme))))
    (if overrides
        (append (symbol-value (jp-themes--palette-symbol theme :overrides))
                jp-themes-common-palette-overrides
                base-value)
      base-value)))

(defun jp-themes--current-theme-palette (&optional overrides)
...
overrides."
  (if-let ((theme (jp-themes--current-theme)))
      (if overrides
          (jp-themes--palette-value theme :overrides)
        (jp-themes--palette-value theme))
    (user-error "No enabled Ef theme could be found")))

(defun jp-themes--choose-subset ()
...
(defun jp-themes--completion-table (category candidates)
...
(defvar jp-themes--select-theme-history nil
...
If SUBSET is neither `light' nor `dark', return all the known jp themes."
  (jp-themes--completion-table
   'theme
   (pcase subset
     ('dark jp-themes-dark-themes)
     ('light jp-themes-light-themes)
     (_ (jp-themes--list-known-themes)))))

(defun jp-themes--maybe-prompt-subset (variant)
  "Helper function for `jp-themes--select-prompt' VARIANT argument."
  (cond
   ((null variant))
   ((or (eq variant 'light) (eq variant 'dark)) variant)
   (t (jp-themes--choose-subset))))

(defun jp-themes--select-prompt (&optional prompt variant)
  "Minibuffer prompt for `jp-themes-select'.
  ...
  (let* ((subset (jp-themes--maybe-prompt-subset variant))
    ...
    (intern
     (completing-read
      ...
      themes
      nil t nil
      'jp-themes--select-theme-history))))

(defun jp-themes--disable-themes ()
  "Disable themes per `jp-themes-disable-other-themes'."
  (mapc #'disable-theme
        (if jp-themes-disable-other-themes
            custom-enabled-themes
          (jp-themes--list-known-themes))))
...
Run the `jp-themes-post-load-hook' as the final step after
loading the THEME.

Return THEME."
  (jp-themes--disable-themes)
  (load-theme theme :no-confirm)
  (run-hooks 'jp-themes-post-load-hook)
  theme)

;;;###autoload
(defun jp-themes-select (theme &optional _variant)
...
Run `jp-themes-post-load-hook' after loading the theme.

When called from Lisp, THEME is the symbol of a theme.  VARIANT
is ignored in this scenario."
  (interactive (list (jp-themes--select-prompt nil current-prefix-arg)))
  (jp-themes--load-theme theme))

;;;###autoload
(defun jp-themes-select-light (theme)
  "Load a light Ef THEME.
Run `jp-themes-post-load-hook' after loading the theme.

Also see `jp-themes-select-dark'.

This command is the same as `jp-themes-select' except it only
prompts for light themes when called interactively.  Calling it
from Lisp behaves the same as `jp-themes-select' for the THEME
argument, meaning that it loads the Ef THEME regardless of
whether it is light or dark."
  (interactive
   (list
    (jp-themes--select-prompt "Select light Ef theme: " 'light)))
  (jp-themes--load-theme theme))

;;;###autoload
(defun jp-themes-select-dark (theme)
  "Load a dark Ef THEME.
Run `jp-themes-post-load-hook' after loading the theme.

Also see `jp-themes-select-light'.
...
whether it is light or dark."
  (interactive
   (list
    (jp-themes--select-prompt "Select dark Ef theme: " 'dark)))
  (jp-themes--load-theme theme))

(defun jp-themes--toggle-theme-p ()
  "Return non-nil if `jp-themes-to-toggle' are valid."
  (mapc (lambda (theme)
          (if (or (memq theme jp-themes-collection)
                  (memq theme (jp-themes--list-known-themes)))
              theme
            (user-error "`%s' is not part of `jp-themes-collection'" theme)))
        jp-themes-to-toggle))
...
Run `jp-themes-post-load-hook' after loading the theme."
  (interactive)
  (if-let* ((themes (jp-themes--toggle-theme-p))
            (one (car themes))
            (two (cadr themes)))
      (if (eq (car custom-enabled-themes) one)
          (jp-themes--load-theme two)
        (jp-themes--load-theme one))
    (jp-themes--load-theme
     (jp-themes--select-prompt
      (concat "Set two `jp-themes-to-toggle'; "
...
`jp-themes--list-known-themes'."
  (let* ((list (when variant
                 (if (eq variant 'dark)
                     jp-themes-dark-themes
                   jp-themes-light-themes)))
         (sequence (or list (jp-themes--list-known-themes)))
         (themes (copy-sequence sequence)))
    (delete (jp-themes--current-theme) themes)))

;;;###autoload
(defun jp-themes-load-random (&optional variant)
...
symbol."
  (interactive (list (when current-prefix-arg (jp-themes--choose-subset))))
  (let* ((themes (jp-themes--minus-current variant))
         (n (random (length themes)))
         (pick (nth n themes))
         (loaded (if (null pick) (car themes) pick)))
    (jp-themes--load-theme loaded)
    (message "Loaded `%s'" loaded)))

(defun jp-themes--preview-colors-render (buffer theme &optional mappings &rest _)
  "Render colors in BUFFER from THEME for `jp-themes-preview-colors'.
Optional MAPPINGS changes the output to only list the semantic
color mappings of the palette, instead of its named colors."
  (let* ((current-palette (jp-themes--palette-value theme mappings))
         (palette (if mappings
    ...
    (with-help-window buffer
      (with-current-buffer standard-output
        ...
        (dolist (cell palette)
          (let* ((name (car cell))
                 (color (jp-themes-get-color-value name mappings theme))
                 (pad (make-string 10 ?\s))
                 (fg (if (eq color 'unspecified)
                         (progn
                           (readable-foreground-color (jp-themes-get-color-value 'bg-main nil theme))
        ...
        (setq-local revert-buffer-function
                    (lambda (_ignore-auto _noconfirm)
                      (jp-themes--preview-colors-render current-buffer current-theme mappings)))))))

(defvar jp-themes--preview-colors-prompt-history '()
  "Minibuffer history for `jp-themes--preview-colors-prompt'.")

(defun jp-themes--preview-colors-prompt ()
  "Prompt for Ef theme.
Helper function for `jp-themes-preview-colors'."
  (let ((def (format "%s" (jp-themes--current-theme)))
        (completion-extra-properties `(:annotation-function ,#'jp-themes--annotate-theme)))
    (completing-read
     (format "Use palette from theme [%s]: " def)
     (jp-themes--load-subset :all-themes)
     nil t nil
     'jp-themes--preview-colors-prompt-history def)))

(defun jp-themes-preview-colors (theme &optional mappings)
...
color mappings instead of the named colors."
  (interactive (list (intern (jp-themes--preview-colors-prompt)) current-prefix-arg))
  (jp-themes--preview-colors-render
   (format (if mappings "*%s-preview-mappings*" "*%s-preview-colors*") theme)
   theme
   mappings))

(defalias 'jp-themes-list-colors 'jp-themes-preview-colors
  "Alias of `jp-themes-preview-colors'.")

(defun jp-themes-preview-colors-current (&optional mappings)
  "Call `jp-themes-list-colors' for the current Ef theme.
Optional prefix argument MAPPINGS has the same meaning as for
`jp-themes-list-colors'."
  (interactive "P")
  (jp-themes-list-colors (jp-themes--current-theme) mappings))

(defalias 'jp-themes-list-colors-current 'jp-themes-preview-colors-current
  "Alias of `jp-themes-preview-colors-current'.")
...
(dotimes (n 9)
  (custom-declare-face
   (intern (format "jp-themes-heading-%d" n))
   nil (format "Used for level %d heading." n)
   :package-version '(jp-themes . "0.3.0")
   :group 'jp-themes-faces))

(defface jp-themes-key-binding nil
  "Face for key bindings."
  :package-version '(jp-themes . "0.3.0")
  :group 'jp-themes-faces)

(defface jp-themes-fixed-pitch nil
  "Face for `fixed-pitch' if `jp-themes-mixed-fonts' is non-nil."
  :package-version '(jp-themes . "0.4.0")
  :group 'jp-themes-faces)

(defface jp-themes-ui-variable-pitch nil
  "Face for `variable-pitch' if `jp-themes-variable-pitch-ui' is non-nil."
  :package-version '(jp-themes . "0.4.0")
  :group 'jp-themes-faces)

(defface jp-themes-reset-soft nil
...
text should not be underlined as well) yet still blend in."
  :package-version '(jp-themes . "1.2.0")
  :group 'jp-themes-faces)

;; This produces `jp-themes-mark-delete' and the like.
(dolist (scope '(delete select other))
  (custom-declare-face
   (intern (format "jp-themes-mark-%s" scope))
   nil (format "Face for %s marks (e.g. `dired', `trashed')." scope)
   :package-version '(jp-themes . "0.9.0")
   :group 'jp-themes-faces))

;; This produces `jp-themes-underline-error' and the like
(dolist (scope '(info error warning))
  (custom-declare-face
   (intern (format "jp-themes-underline-%s" scope))
   nil (format "Face for %s underline (e.g. `flymake', `flyspell')." scope)
   :package-version '(jp-themes . "0.9.0")
   :group 'jp-themes-faces))

;; This produces `jp-themes-search-current' and the like
(dolist (scope '(current lazy replace))
  (custom-declare-face
   (intern (format "jp-themes-search-%s" scope))
   nil (format "Search of type %s." scope)
   :package-version '(jp-themes . "1.5.0")
   :group 'jp-themes-faces))

;; This produces `jp-themes-search-rx-group-0' and the like
(dotimes (n 4)
  (custom-declare-face
   (intern (format "jp-themes-search-rx-group-%s" n))
   nil (format "Search regexp group number %s." n)
   :package-version '(jp-themes . "1.5.0")
   :group 'jp-themes-faces))

(defconst jp-themes-faces
  '(
;;;; internal faces
    `(jp-themes-fixed-pitch ((,c ,@(jp-themes--fixed-pitch))))
    `(jp-themes-heading-0 ((,c ,@(jp-themes--heading 0) :foreground ,rainbow-0)))
    `(jp-themes-heading-1 ((,c ,@(jp-themes--heading 1) :foreground ,fg-dim)))
    `(jp-themes-heading-2 ((,c ,@(jp-themes--heading 2) :foreground ,fg-alt)))
    `(jp-themes-heading-3 ((,c ,@(jp-themes--heading 3) :foreground ,rainbow-3)))
    `(jp-themes-heading-4 ((,c ,@(jp-themes--heading 4) :foreground ,rainbow-4)))
    `(jp-themes-heading-5 ((,c ,@(jp-themes--heading 5) :foreground ,rainbow-5)))
    `(jp-themes-heading-6 ((,c ,@(jp-themes--heading 6) :foreground ,rainbow-6)))
    `(jp-themes-heading-7 ((,c ,@(jp-themes--heading 7) :foreground ,rainbow-7)))
    `(jp-themes-heading-8 ((,c ,@(jp-themes--heading 8) :foreground ,rainbow-8)))
    `(jp-themes-key-binding ((,c :inherit (bold jp-themes-fixed-pitch) :foreground ,keybind)))
    `(jp-themes-ui-variable-pitch ((,c ,@(jp-themes--variable-pitch-ui))))
    `(jp-themes-mark-delete ((,c :inherit error :background ,bg-err)))
    `(jp-themes-mark-select ((,c :inherit success :background ,bg-info)))
    `(jp-themes-mark-other ((,c :inherit warning :background ,bg-warning)))
    `(jp-themes-search-current ((,c :background ,bg-search-current :foreground ,fg-intense)))
    `(jp-themes-search-lazy ((,c :background ,bg-search-lazy :foreground ,fg-intense)))
    `(jp-themes-search-replace ((,c :background ,bg-search-replace :foreground ,fg-intense)))
    `(jp-themes-search-rx-group-0 ((,c :background ,bg-search-rx-group-0 :foreground ,fg-intense)))
    `(jp-themes-search-rx-group-1 ((,c :background ,bg-search-rx-group-1 :foreground ,fg-intense)))
    `(jp-themes-search-rx-group-2 ((,c :background ,bg-search-rx-group-2 :foreground ,fg-intense)))
    `(jp-themes-search-rx-group-3 ((,c :background ,bg-search-rx-group-3 :foreground ,fg-intense)))
    `(jp-themes-underline-error ((,c :underline (:style wave :color ,underline-err))))
    `(jp-themes-underline-info ((,c :underline (:style wave :color ,underline-info))))
    `(jp-themes-underline-warning ((,c :underline (:style wave :color ,underline-warning))))
    `(jp-themes-reset-soft ((,c :background ,bg-main :foreground ,fg-main
...
;;;;; all other basic faces
    ...
    `(header-line ((,c :inherit jp-themes-ui-variable-pitch :background ,bg-dim)))
    `(header-line-highlight ((,c :inherit highlight)))
    `(help-argument-name ((,c :foreground ,accent-0)))
    `(help-key-binding ((,c :inherit (bold jp-themes-fixed-pitch) :foreground ,keybind)))
...
;;;; auctex and tex
    ...
    `(font-latex-sectioning-5-face ((,c :inherit (bold jp-themes-variable-pitch) :foreground ,fg-alt)))
    `(font-latex-sedate-face ((,c :inherit font-lock-keyword-face)))
    `(font-latex-slide-title-face ((,c :inherit jp-themes-heading-0)))
    `(font-latex-string-face ((,c :inherit font-lock-string-face)))
    `(font-latex-underline-face ((,c :inherit underline)))
    `(font-latex-verbatim-face ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-verbatim)))
    `(font-latex-warning-face ((,c :inherit font-lock-warning-face)))
    `(tex-verbatim ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-verbatim)))
...
;;;; avy
    ...
    `(avy-lead-face ((,c :inherit (bold jp-themes-reset-soft) :background ,bg-char-0)))
    `(avy-lead-face-0 ((,c :inherit (bold jp-themes-reset-soft) :background ,bg-char-1)))
    `(avy-lead-face-1 ((,c :inherit jp-themes-reset-soft :background ,bg-inactive)))
    `(avy-lead-face-2 ((,c :inherit (bold jp-themes-reset-soft) :background ,bg-char-2)))
;;;; aw (ace-window)
    `(aw-background-face ((,c :foreground ,fg-dim)))
    `(aw-key-face ((,c :inherit jp-themes-key-binding)))
    `(aw-leading-char-face ((,c :inherit (bold jp-themes-reset-soft) :height 1.5 :foreground ,keybind)))
    `(aw-minibuffer-leading-char-face ((,c :inherit jp-themes-key-binding)))
...
;;;; bongo
    ...
    `(bongo-marked-track ((,c :inherit jp-themes-mark-other)))
...
;;;; calfw
	`(cfw:face-title ((,c (:foreground "#f0dfaf" :weight bold :height 2.0 :inherit variable-pitch))))
	`(cfw:face-header ((,c (:foreground "#d0bf8f" :weight bold))))
	`(cfw:face-sunday ((,c :foreground "#cc9393" :background "grey10" :weight bold)))
	`(cfw:face-saturday ((,c :foreground "#8cd0d3" :background "grey10" :weight bold)))
	`(cfw:face-holiday ((,c :background "grey10" :foreground "#8c5353" :weight bold)))
	`(cfw:face-grid ((,c :foreground "DarkGrey")))
	`(cfw:face-default-content ((,c :foreground "#bfebbf")))
	`(cfw:face-periods ((,c :foreground "cyan")))
	`(cfw:face-day-title ((,c :background ,fg-main)))
	`(cfw:face-default-day ((,c :weight bold :inherit cfw:face-day-title)))
	`(cfw:face-annotation ((,c :foreground "RosyBrown" :inherit cfw:face-day-title)))
	`(cfw:face-disable ((,c :foreground "DarkGray" :inherit cfw:face-day-title)))
	`(cfw:face-today-title ((,c :background "#7f9f7f" :weight bold)))
	`(cfw:face-today ((,c :background: "grey10" :weight bold)))
	`(cfw:face-select ((,c :background "#2f2f2f")))
	`(cfw:face-toolbar ((,c :foreground "Steelblue4" :background "Steelblue4")))
	`(cfw:face-toolbar-button-off ((,c :foreground "Gray10" :weight bold)))
	`(cfw:face-toolbar-button-on ((,c :foreground "Gray50" :weight bold)))
;;;; centaur-tabs
...
;;;; cider
	...
	`(cider-error-highlight-face ((,c :inherit jp-themes-underline-error)))
	`(cider-fringe-good-face ((,c :inherit jp-themes-mark-select)))
	...
	`(cider-warning-highlight-face ((,c :inherit jp-themes-underline-warning)))
...
;;;; consult
	...
	`(consult-key ((,c :inherit jp-themes-key-binding)))
...
;;;; eglot
	...
	`(eglot-diagnostic-tag-unnecessary-face ((,c :inherit jp-themes-underline-info)))
;;;; eldoc
...
;;;; embark
	`(embark-keybinding ((,c :inherit jp-themes-key-binding)))
	`(embark-keybinding-repeat ((,c :inherit bold)))
	`(embark-collect-marked ((,c :inherit jp-themes-mark-select)))
...
;;;; flycheck
	`(flycheck-error ((,c :inherit jp-themes-underline-error)))
	`(flycheck-fringe-error ((,c :inherit jp-themes-mark-delete)))
	`(flycheck-fringe-info ((,c :inherit jp-themes-mark-select)))
	`(flycheck-fringe-warning ((,c :inherit jp-themes-mark-other)))
	`(flycheck-info ((,c :inherit jp-themes-underline-info)))
	`(flycheck-warning ((,c :inherit jp-themes-underline-warning)))
;;;; flymake
	`(flymake-end-of-line-diagnostics-face ((,c :inherit italic :height 0.85 :box ,border)))
	`(flymake-error ((,c :inherit jp-themes-underline-error)))
	...
	`(flymake-note ((,c :inherit jp-themes-underline-info)))
	...
	`(flymake-warning ((,c :inherit jp-themes-underline-warning)))
...
;;;; flyspell
	`(flyspell-duplicate ((,c :inherit jp-themes-underline-warning)))
	`(flyspell-incorrect ((,c :inherit jp-themes-underline-error)))
;;;; font-lock
...
;;;; git-gutter
	`(git-gutter:added ((,c :background ,bg-added :foreground ,bg-added :weight bold)))
	`(git-gutter:deleted ((,c :background ,bg-removed :foreground ,bg-removed :weight bold)))
	`(git-gutter:modified ((,c :background ,bg-changed :foreground ,bg-changed :weight bold)))
	`(git-gutter:separator ((,c :inherit success)))
	`(git-gutter:unchanged ((,c :inherit bold)))
;;;; git-gutter-fr
	`(git-gutter-fr:added ((,c :background ,bg-added :foreground ,bg-added)))
	`(git-gutter-fr:deleted ((,c :background ,bg-removed :foreground ,bg-removed)))
	`(git-gutter-fr:modified ((,c :background ,bg-changed :foreground ,bg-changed)))
;;;; git-rebase
...
;;;; highlight-thing
	`(highlight-thing ((,c :underline t)))
	`(lazy-highlight ((,c :background ,bg-alt)))
;;;; ibuffer
...
;;;; info
	`(Info-quoted ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-verbatim))) ; the capitalization is canonical
	...
	`(info-title-1 ((,c :inherit jp-themes-heading-1)))
	`(info-title-2 ((,c :inherit jp-themes-heading-2)))
	`(info-title-3 ((,c :inherit jp-themes-heading-3)))
	`(info-title-4 ((,c :inherit jp-themes-heading-4)))
;;;; isearch, occur, and the like
	`(isearch ((,c :inherit jp-themes-search-current)))
	`(isearch-fail ((,c :inherit error :background ,bg-err :foreground ,err)))
	`(isearch-group-1 ((,c :inherit jp-themes-search-rx-group-0)))
	`(isearch-group-2 ((,c :inherit jp-themes-search-rx-group-1)))
	`(lazy-highlight ((,c :inherit jp-themes-search-lazy)))
	`(match ((,c :background ,bg-warning)))
	`(query-replace ((,c :inherit jp-themes-search-replace)))
;;;; jit-spell
	`(jit-spell-misspelling ((,c :inherit jp-themes-underline-error)))
;;;; jinx
	`(jinx-misspelled ((,c :inherit jp-themes-underline-warning)))
;;;; keycast
...
;;;; line numbers (display-line-numbers-mode and global variant)
	...
	`(line-number ((,c :inherit (jp-themes-fixed-pitch shadow default))))
...
;;;; lsp-ui
	`(lsp-ui-doc-header ((,c :background ,fg-alt)))
;;;; magit
	...
	`(magit-header-line-key ((,c :inherit jp-themes-key-binding)))
	...
	`(magit-section-highlight ((,c :background ,bg-main)))
...
;;;; marginalia
	...
	`(marginalia-key ((,c :inherit jp-themes-key-binding)))
...
;;;; markdown-mode
	...
	`(markdown-code-face ((,c :inherit jp-themes-fixed-pitch :background ,bg-inactive :extend t)))
	`(markdown-gfm-checkbox-face ((,c :foreground ,warning)))
	`(markdown-header-face (( )))
	`(markdown-header-face-1 ((,c :inherit jp-themes-heading-1)))
	`(markdown-header-face-2 ((,c :inherit jp-themes-heading-2)))
	`(markdown-header-face-3 ((,c :inherit jp-themes-heading-3)))
	`(markdown-header-face-4 ((,c :inherit jp-themes-heading-4)))
	`(markdown-header-face-5 ((,c :inherit jp-themes-heading-5)))
	`(markdown-header-face-6 ((,c :inherit jp-themes-heading-6)))
	`(markdown-highlighting-face ((,c :background ,bg-info :foreground ,info)))
	`(markdown-inline-code-face ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-code)))
	`(markdown-italic-face ((,c :inherit italic)))
	`(markdown-language-keyword-face ((,c :inherit jp-themes-fixed-pitch :background ,bg-dim)))
	...
	`(markdown-table-face ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-table)))
...
;;;; mode-line
	`(mode-line ((,c :inherit jp-themes-ui-variable-pitch :background ,bg-mode-line :foreground ,fg-mode-line)))
	`(mode-line-active ((,c :inherit mode-line)))
	`(mode-line-buffer-id ((,c :inherit bold)))
	`(mode-line-emphasis ((,c :inherit bold :foreground ,modeline-info)))
	`(mode-line-highlight ((,c :inherit highlight)))
	`(mode-line-inactive ((,c :inherit jp-themes-ui-variable-pitch :background ,bg-alt :foreground ,fg-dim)))
;;;; mood-line
...
;;;; mu4e
	...
	`(mu4e-highlight-face ((,c :inherit jp-themes-key-binding)))
...
;;;; notmuch
	...
	`(notmuch-jump-key ((,c :inherit jp-themes-key-binding)))
...
;;;; org
	...
	`(org-agenda-date ((,c ,@(jp-themes--heading 'agenda-date) :foreground ,date-weekday)))
	...
	`(org-agenda-structure ((,c ,@(jp-themes--heading 'agenda-structure) :foreground ,fg-alt)))
	...
	`(org-block ((,c :inherit jp-themes-fixed-pitch :background ,bg-dim :extend t)))
	`(org-block-begin-line ((,c :inherit (shadow jp-themes-fixed-pitch) :background ,bg-main :extend t)))
	`(org-block-end-line ((,c :inherit org-block-begin-line)))
	`(org-checkbox ((,c :background ,bg-main
						:foreground ,fg-alt
						:box (:line-width 2 :color ,fg-dim
                                          :style released-button))))
	`(org-checkbox-statistics-done ((,c :inherit org-done)))
	`(org-checkbox-statistics-todo ((,c :inherit org-todo)))
	`(org-clock-overlay ((,c :background ,bg-hover-secondary)))
	`(org-code ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-code)))
	`(org-column ((,c :inherit default :background ,bg-alt)))
	`(org-column-title ((,c :inherit (bold default) :underline t :background ,bg-alt)))
	`(org-date ((,c :inherit jp-themes-fixed-pitch :foreground ,date-common)))
	...
	`(org-formula ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-table-formula)))
	...
	`(org-level-1 ((,c :inherit jp-themes-heading-1)))
	`(org-level-2 ((,c :inherit jp-themes-heading-2)))
	`(org-level-3 ((,c :inherit jp-themes-heading-3)))
	`(org-level-4 ((,c :inherit jp-themes-heading-4)))
	`(org-level-5 ((,c :inherit jp-themes-heading-5)))
	`(org-level-6 ((,c :inherit jp-themes-heading-6)))
	`(org-level-7 ((,c :inherit jp-themes-heading-7)))
	`(org-level-8 ((,c :inherit jp-themes-heading-8)))
	`(org-link ((,c :inherit link)))
	`(org-list-dt ((,c :inherit bold)))
	`(org-macro ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-macro)))
	`(org-meta-line ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-metadata)))
	`(org-mode-line-clock (( )))
	`(org-mode-line-clock-overrun ((,c :inherit bold :foreground ,modeline-err)))
	`(org-priority ((,c :foreground ,prose-tag)))
	`(org-property-value ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-metadata-value)))
	...
	`(org-special-keyword ((,c :inherit (shadow jp-themes-fixed-pitch))))
	`(org-table ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-table)))
	...
	`(org-verbatim ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-verbatim)))
...
;;;; org-modern
	`(org-modern-date-active ((,c :inherit (jp-themes-fixed-pitch org-modern-label) :background ,bg-alt)))
	`(org-modern-date-inactive ((,c :inherit (jp-themes-fixed-pitch org-modern-label) :background ,bg-dim :foreground ,fg-dim)))
	...
	`(org-modern-time-active ((,c :inherit (jp-themes-fixed-pitch org-modern-label) :background ,bg-active :foreground ,fg-intense)))
...
;;;; outline-mode
	`(outline-1 ((,c :inherit jp-themes-heading-1)))
	`(outline-2 ((,c :inherit jp-themes-heading-2)))
	`(outline-3 ((,c :inherit jp-themes-heading-3)))
	`(outline-4 ((,c :inherit jp-themes-heading-4)))
	`(outline-5 ((,c :inherit jp-themes-heading-5)))
	`(outline-6 ((,c :inherit jp-themes-heading-6)))
	`(outline-7 ((,c :inherit jp-themes-heading-7)))
	`(outline-8 ((,c :inherit jp-themes-heading-8)))
;;;; outline-minor-faces
	`(outline-minor-0 (( )))
...
;;;; regexp-builder (re-builder)
	`(reb-match-0 ((,c :inherit jp-themes-search-rx-group-0)))
	`(reb-match-1 ((,c :inherit jp-themes-search-rx-group-1)))
	`(reb-match-2 ((,c :inherit jp-themes-search-rx-group-2)))
	`(reb-match-3 ((,c :inherit jp-themes-search-rx-group-3)))
...
;;;; rst-mode
	`(rst-level-1 ((,c :inherit jp-themes-heading-1)))
	`(rst-level-2 ((,c :inherit jp-themes-heading-2)))
	`(rst-level-3 ((,c :inherit jp-themes-heading-3)))
	`(rst-level-4 ((,c :inherit jp-themes-heading-4)))
	`(rst-level-5 ((,c :inherit jp-themes-heading-5)))
	`(rst-level-6 ((,c :inherit jp-themes-heading-6)))
;;;; ruler-mode
...
;;;; shr
	`(shr-code ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-code)))
	`(shr-h1 ((,c :inherit jp-themes-heading-1)))
	`(shr-h2 ((,c :inherit jp-themes-heading-2)))
	`(shr-h3 ((,c :inherit jp-themes-heading-3)))
	`(shr-h4 ((,c :inherit jp-themes-heading-4)))
	`(shr-h5 ((,c :inherit jp-themes-heading-5)))
	`(shr-h6 ((,c :inherit jp-themes-heading-6)))
...
;;;; tab-bar-mode
	`(tab-bar ((,c :inherit jp-themes-ui-variable-pitch :background ,bg-tab-bar)))
...
;;;; tab-line-mode
	`(tab-line ((,c :inherit jp-themes-ui-variable-pitch :background ,bg-tab-bar :height 0.95)))
...
;;;; telega
	`(telega-entity-type-code ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-code)))
	`(telega-delim-face ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-code)))
	`(telega-webpage-fixed ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-code)))
	`(telega-entity-type-pre ((,c :inherit jp-themes-fixed-pitch :foreground ,prose-code)))
;;;; tempel
...
;;;; transient
	...
	`(transient-key ((,c :inherit jp-themes-key-binding)))
	`(transient-key-exit ((,c :inherit jp-themes-key-binding)))
	`(transient-key-noop ((,c :inherit (shadow jp-themes-key-binding))))
	`(transient-key-return ((,c :inherit jp-themes-key-binding)))
	`(transient-key-stay ((,c :inherit jp-themes-key-binding)))
...
;;;; trashed
	`(trashed-restored ((,c :inherit jp-themes-mark-other)))
;;;; tree-sitter
...
;;;; which-key
	...
	`(which-key-key-face ((,c :inherit jp-themes-key-binding)))
...
;;;; writegood-mode
	`(writegood-duplicates-face ((,c :inherit jp-themes-underline-error)))
	`(writegood-passive-voice-face ((,c :inherit jp-themes-underline-info)))
	`(writegood-weasels-face ((,c :inherit jp-themes-underline-warning)))
;;;; woman
...
;;;; ztree
	...
	`(ztreep-diff-header-face ((,c :inherit jp-themes-heading-0)))
	...
	`(ztreep-header-face ((,c :inherit jp-themes-heading-0)))
	`(ztreep-leaf-face (( )))
	`(ztreep-node-count-children-face ((,c :inherit (shadow italic))))
	`(ztreep-node-face ((,c :foreground ,accent-0))))
  "Face specs for use with `jp-themes-theme'.")

(defconst jp-themes-custom-variables
  '(
;;;; chart
...
;;;; flymake fringe indicators
    `(flymake-error-bitmap '(flymake-double-exclamation-mark jp-themes-mark-delete))
    `(flymake-warning-bitmap '(exclamation-mark jp-themes-mark-other))
    `(flymake-note-bitmap '(exclamation-mark jp-themes-mark-select))
;;;; ibuffer
    `(ibuffer-deletion-face 'jp-themes-mark-delete)
    `(ibuffer-filter-group-name-face 'bold)
    `(ibuffer-marked-face 'jp-themes-mark-select)
    `(ibuffer-title-face 'default))
  "Custom variables for `jp-themes-theme'.")
...
;;;###autoload
(defmacro jp-themes-theme (name palette &optional overrides)
  "Bind NAME's color PALETTE around face specs and variables.
Face specifications are passed to `custom-theme-set-faces'.
While variables are handled by `custom-theme-set-variables'.
Those are stored in `jp-themes-faces' and
`jp-themes-custom-variables' respectively.
...
corresponding entries."
  ...
  (let ((sym (gensym))
    ...
    `(let* ((c '((class color) (min-colors 256)))
            (,sym (jp-themes--palette-value ',name ',overrides))
            ,@(mapcar (lambda (color)
                        (list color
                              `(jp-themes--retrieve-palette-value ',color ,sym)))
                      colors))
       (custom-theme-set-faces ',name ,@jp-themes-faces)
       (custom-theme-set-variables ',name ,@jp-themes-custom-variables))))

;;;; Use theme colors

(defmacro jp-themes-with-colors (&rest body)
  ...
  (let* ((sym (gensym))
         ...
         ;; inside a function.
         (colors (mapcar #'car (jp-themes--current-theme-palette))))
    `(let* ((c '((class color) (min-colors 256)))
            (,sym (jp-themes--current-theme-palette :overrides))
            ,@(mapcar (lambda (color)
                        (list color
                              `(jp-themes--retrieve-palette-value ',color ,sym)))
                      colors))
